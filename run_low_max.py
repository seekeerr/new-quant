"""
ANTI-LOTTERY / LOW-MAX FACTOR — A/B/C/D on the honest (survivorship-free) universe.

NEW FACTOR (the only thing introduced):
    Low-MAX (anti-lottery)
        MAX   = maximum single-day return over the last 21 trading days
        score = -MAX            (LOWER MAX is better -> higher score)
...
Why this is worth testing (Bali, Cakici & Whitelaw 2011, JFE, "Maxing Out"):
investors overpay for lottery-like stocks with a recent extreme up-day, which then
underperform. Avoiding high-MAX names earns a premium that is documented to be
distinct from plain volatility, and evidence suggests it is *stronger* in
retail-heavy markets like India.
...
C and D differ from each other ONLY in the ranking leg (12-1 momentum vs Low-MAX), so
the comparison isolates the factor and nothing else.

FOUR VARIANTS (Top10 / quarterly / Buffer20 / NET):
  A. Momentum          — validated 12-1 momentum (reference baseline)
  B. Low-MAX           — the new anti-lottery factor, standalone
  C. Mom + LowVol      — the CURRENT VALIDATED CHAMPION (the bar to beat)
  D. Low-MAX + LowVol  — the candidate upgrade
...
repaired this session in data/benchmark.py, so Beta and Jensen's Alpha here are valid.

Outputs (results/factor_low_max/): comparison.csv, concentration.csv, report.txt,
...
MAX_LOOKBACK = 21                       # MAX = max daily return over last 21 trading days
DD_TOLERANCE = 0.03                     # "materially worse drawdown" = >3.0 pts deeper
CACHE_BHAV = PROJECT_ROOT / "data" / "cache_bhav"

CHAMP = "C. Mom + LowVol"
CAND = "D. Low-MAX + LowVol"

PALETTE = {
    "A. Momentum":         "#ff6b6b",
    "B. Low-MAX":          "#ffd93d",
    "C. Mom + LowVol":     "#00ff88",   # champion
    "D. Low-MAX + LowVol": "#4ecdc4",   # candidate upgrade
}


# ─────────────────────────────────────────────────────────────────────
# NEW FACTOR — LOW-MAX / ANTI-LOTTERY  (price-only, drop-in scorer)
# ─────────────────────────────────────────────────────────────────────
def compute_low_max(close_panel, date, universe, lookback=MAX_LOOKBACK):
    """MAX = maximum single-day return over the last `lookback` trading days.

    Lower MAX is better, so score = -MAX (higher = better, sorted best->worst).
    Self-contained on the same close panel every scorer here receives."""
    cols = [s for s in universe if s in close_panel.columns]
    prices = close_panel.loc[close_panel.index <= date, cols]
    if len(prices) < lookback + 1:
        return pd.Series(dtype=float)
    rets = prices.tail(lookback + 1).pct_change().tail(lookback)   # last 21 daily returns
    mx = rets.max()                                                # the single biggest up-day
    score = -mx                                                    # lower MAX => higher score
...
# differs, so D isolates the Low-MAX factor against the champion.
def build_specs():
    return [
        ("A. Momentum",         compute_12_1_momentum),
        ("B. Low-MAX",          compute_low_max),
        ("C. Mom + LowVol",     make_blend_scorer(compute_12_1_momentum, 0.5, VOL_LOOKBACK)),
        ("D. Low-MAX + LowVol", make_blend_scorer(compute_low_max, 0.5, VOL_LOOKBACK)),
    ]
...
def main():
    ...
    print("=" * 100)
    print("  LOW-MAX / ANTI-LOTTERY — A/B/C/D on the honest universe (Top10/quarterly/Buffer20, NET)".center(100))
    print("=" * 100)
    ...
    # ── OUTPUTS ──
    rdir = RESULTS_DIR / "factor_low_max"
...
def write_report(rdir, names, headline, grid, split, rolling, d_beats_c,
    ...
    w("=" * W)
    w("LOW-MAX / ANTI-LOTTERY FACTOR — SURVIVORSHIP-FREE UNIVERSE".center(W))
    w("Top10 / Quarterly / Equal Weight / Buffer20 / Net of Indian costs".center(W))
    w("=" * W)
    w(f"Initial Capital: Rs {cap:,.0f}   Period: {bstart}..{bend}")
    w(f"New factor: MAX = max daily return over last {MAX_LOOKBACK} trading days; score = -MAX (lower MAX better).")
    ...
    w(f"Benchmark date-parse bug fixed this session -> Beta/Alpha are valid.")
    w("")
    ...
    w("Pre-registered: D (Low-MAX+LowVol) improves Sharpe AND/OR CAGR vs C (champion),")
    ...
    w("commensurate Sharpe gain; watch whether Low-MAX behaves the same.")
    w("=" * W)
...
def make_charts(names, headline, bench_eq, bm_annual, save_path):
    ...
    fig.suptitle("Low-MAX / Anti-Lottery vs Momentum / LowVol — Survivorship-Free Universe\n"
...
def write_walkthrough(rdir, names, headline, grid, split, rolling, d_beats_c,
    ...
    L = []; w = L.append
    w("# Low-MAX / Anti-Lottery — Factor Experiment Walkthrough\n")
    w(f"_Period {bstart} .. {bend}. Initial capital Rs {cap:,.0f}. "
      f"All figures **net** of the Indian delivery cost model._\n")

    w("## 1. What was tested\n")
    w("A single new **price-only** ranking factor, **Low-MAX (anti-lottery)**:\n")
    w("```")
    w(f"MAX   = maximum single-day return over the last {MAX_LOOKBACK} trading days")
    w("score = -MAX            (lower MAX = better rank)")
    w("```")
    w("Per Bali, Cakici & Whitelaw (2011, *JFE*, \"Maxing Out\"), investors overpay "
      "for lottery-like stocks that just printed an extreme up-day; those names "
      "subsequently underperform. Avoiding them earns a premium documented to be "
      "distinct from plain volatility, and plausibly **stronger in retail-heavy India**.\n")
    ...
    w("\nVariants **C** and **D** are produced by the *same* validated 50/50 "
      ...
      "ranking leg (12-1 momentum vs Low-MAX). So D minus C isolates the factor.\n")
    w("> Note: the benchmark CSV date-parse bug found in the prior experiment was "
      "repaired this session (`data/benchmark.py`), so **Beta and Jensen's Alpha "
      "below are valid** (benchmark daily vol back to ~16%).\n")
    ...
    w("\n## 8. Caveats\n")
    w(f"- Low-MAX uses the **single** maximum daily return over the last "
      f"{MAX_LOOKBACK} days, exactly as specified. The literature's MAX5 (mean of the "
      "5 largest up-days) is an alternative spec, not a tuning parameter — left for a "
      "separate robustness run.\n")
    ...
    w("py run_low_max.py")
    w("```")
    w("Outputs: `results/factor_low_max/{comparison.csv, concentration.csv, "
