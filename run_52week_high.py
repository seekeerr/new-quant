"""
52-WEEK-HIGH PROXIMITY FACTOR — A/B/C/D on the honest (survivorship-free) universe.

NEW FACTOR (the only thing introduced):
    52-Week High Proximity
        score = current_close / max(close over the last 252 trading days)
    Range (0, 1]; 1.0 == the stock is AT its 1-year high. Higher = better rank.
    Price-only, survivorship-free, no new data. Computed on the SAME close panel
    every other scorer in this repo uses, so it is a pure drop-in ranking signal.

Why this is the right next test (George & Hwang 2004, JoF): the 52-week-high anchor
is a level signal, not a path-dependent cumulative return, so it carries momentum's
under-reaction edge with documented LOWER crash risk — a direct attack on the -56%
drawdown we measured on pure 12-1 momentum.
...
C and D differ from each other ONLY in the ranking leg (12-1 momentum vs 52WH), so the
comparison isolates the factor and nothing else.

FOUR VARIANTS (Top10 / quarterly / Buffer20 / NET):
  A. Pure Momentum     — validated 12-1 momentum (reference baseline)
  B. 52-Week High      — the new factor, standalone
  C. Mom + LowVol      — the CURRENT VALIDATED CHAMPION (the bar to beat)
  D. 52WH + LowVol     — the candidate upgrade

PRE-REGISTERED SUCCESS CRITERION (stated before running):
  Variant D improves Sharpe AND/OR CAGR vs Variant C WITHOUT materially worsening
  drawdown. "Materially worse" is fixed up front as MaxDD more than 3.0 pts deeper.
...
VALIDATION (no parameter search anywhere):
  ...
  - Concentration : the SAME four variants at Top3 / Top5 / Top10, reported purely
                    as a robustness read. Top10 stays the headline; N is NOT optimized.

Outputs (results/factor_52week_high/): comparison.csv, report.txt, report.png,
WALKTHROUGH.md.
"""
...
LB52 = 252                              # 52-week (1-year) high window
DD_TOLERANCE = 0.03                     # "materially worse drawdown" = >3.0 pts deeper
CACHE_BHAV = PROJECT_ROOT / "data" / "cache_bhav"

# Beta/Alpha are reported (as requested) but flagged: the benchmark DAILY-return
# series is corrupted upstream and is NOT part of anything tuned here.
BENCH_NOTE = (
    "(*) Beta/Alpha are UNRELIABLE: the benchmark daily-return series "
    "(get_benchmark_returns) is corrupted -- annualised vol ~77% with spurious "
    "+/-30-40% daily jumps from a date-parse ambiguity (dayfirst/mixed) in "
    "nifty500_tri.csv. This inflates benchmark variance, collapsing Beta toward 0 "
    "and overstating Jensen's Alpha (~ CAGR - rf). It is a PRE-EXISTING benchmark "
    "data issue (also present in earlier Top10 runs); the engine, costs, universe, "
    "and benchmark methodology were NOT modified. The CAGR-endpoint benchmark is "
    "unaffected, so 'Excess vs Bmk (CAGR)' is the trustworthy relative metric."
)
...
PALETTE = {
    "A. Pure Momentum": "#ff6b6b",
    "B. 52-Week High":  "#ffd93d",
    "C. Mom + LowVol":  "#00ff88",      # champion
    "D. 52WH + LowVol": "#4ecdc4",      # candidate upgrade
}


# ─────────────────────────────────────────────────────────────────────
# NEW FACTOR — 52-WEEK-HIGH PROXIMITY  (price-only, drop-in scorer)
# ─────────────────────────────────────────────────────────────────────
def compute_52w_high(close_panel, date, universe, lookback=LB52):
    """score = current_close / max(close over last `lookback` trading days).

    In (0, 1]; 1.0 means the name is sitting at its 1-year high. Higher = better.
    Uses daily CLOSES for both the current price and the high (self-contained on the
    same close panel every scorer here receives). Returns best->worst."""
    cols = [s for s in universe if s in close_panel.columns]
    prices = close_panel.loc[close_panel.index <= date, cols]
    if len(prices) < lookback + 1:
        return pd.Series(dtype=float)
    window = prices.tail(lookback)                  # last 252 trading days incl. today
    high_252 = window.max()
    score = window.iloc[-1] / high_252.replace(0, np.nan)
    return score.replace([np.inf, -np.inf], np.nan).dropna().sort_values(ascending=False)


# Variant -> ranking signal. C and D share make_blend_scorer; only the momentum leg
# differs, so D isolates the 52WH factor against the champion.
def build_specs():
    return [
        ("A. Pure Momentum", compute_12_1_momentum),
        ("B. 52-Week High",  compute_52w_high),
        ("C. Mom + LowVol",  make_blend_scorer(compute_12_1_momentum, 0.5, VOL_LOOKBACK)),
        ("D. 52WH + LowVol", make_blend_scorer(compute_52w_high, 0.5, VOL_LOOKBACK)),
    ]
...
def main():
    ...
    print("=" * 100)
    print("  52-WEEK-HIGH PROXIMITY — A/B/C/D on the honest universe (Top10/quarterly/Buffer20, NET)".center(100))
    print("=" * 100)
    ...
    #    smaller N are a robustness read only. We capture the N=10 net runs in full
    #    (equity curves) for the split-sample / rolling validations + charts. ──
    grid = {}            # (name, n) -> metrics dict
    headline = {}        # name -> {net, gross, m, gm, extra, roll, annual}
    ...
    # ── Benchmark aligned to the strategy's realized span (warmup pushes start out). ──
    ...
    bm_h1 = subperiod(bench_eq.iloc[:bench_eq.index.get_indexer([mid_date], method="nearest")[0] + 1], None, cap) \
        if not bench_eq.empty else None
    bm_h2 = subperiod(bench_eq.iloc[bench_eq.index.get_indexer([mid_date], method="nearest")[0]:], None, cap) \
        if not bench_eq.empty else None
...
def verdict_lines(headline):
    C = headline["C. Mom + LowVol"]["m"]
    D = headline["D. 52WH + LowVol"]["m"]
    dcagr = D.cagr - C.cagr
...
def write_report(rdir, names, headline, grid, split, rolling, d_beats_c,
    ...
    w("=" * W)
    w("52-WEEK-HIGH PROXIMITY FACTOR — SURVIVORSHIP-FREE UNIVERSE".center(W))
    w("Top10 / Quarterly / Equal Weight / Buffer20 / Net of Indian costs".center(W))
    w("=" * W)
    w(f"Initial Capital: Rs {cap:,.0f}   Period: {bstart}..{bend}")
    w(f"New factor: 52WH = close / max(close, last {LB52} trading days). Higher = better.")
    ...
    w("-" * W)
    for chunk in _wrap(BENCH_NOTE, W):
        w(chunk)
    ...
    w("Pre-registered: D (52WH+LowVol) improves Sharpe AND/OR CAGR vs C (champion),")
    ...
    w(f"  => VERDICT: {'PASS' if passed else 'FAIL'} — "
      + ("D clears the bar." if passed else "D does not clear the bar."))
    w("=" * W)
    ...
    w("-" * W)
    dc1 = split["D. 52WH + LowVol"]["h1"].sharpe_ratio - split["C. Mom + LowVol"]["h1"].sharpe_ratio
    dc2 = split["D. 52WH + LowVol"]["h2"].sharpe_ratio - split["C. Mom + LowVol"]["h2"].sharpe_ratio
    ...
    w("commensurate Sharpe gain; watch whether 52WH behaves the same.")
    w("=" * W)
...
# ─────────────────────────────────────────────────────────────────────
# REPORT.PNG  (mirrors the survivorship-free report layout)
# ─────────────────────────────────────────────────────────────────────
def make_charts(names, headline, bench_eq, bm_annual, save_path):
    plt.style.use("dark_background")
    fig = plt.figure(figsize=(18, 24))
    gs = GridSpec(5, 2, figure=fig, hspace=0.40, wspace=0.22)
    fig.suptitle("52-Week-High Proximity vs Momentum / LowVol — Survivorship-Free Universe\n"
...
def write_walkthrough(rdir, names, headline, grid, split, rolling, d_beats_c,
    ...
    def mrow(n):
        ...
        return (f"| {n} | {m.cagr:.1%} | {m.max_drawdown:.1%} | {m.sharpe_ratio:.2f} | "
                f"{m.calmar_ratio:.2f} | {m.alpha:+.1%} | {m.annualised_volatility:.1%} | "
                f"{m.beta:.2f} | {net['total_trades']} | {net['avg_turnover']:.0%} | "
                f"{net['total_costs']:,.0f} |")

    L = []; w = L.append
    w("# 52-Week-High Proximity — Factor Experiment Walkthrough\n")
    w(f"_Period {bstart} .. {bend}. Initial capital Rs {cap:,.0f}. "
      f"All figures **net** of the Indian delivery cost model._\n")

    w("## 1. What was tested\n")
    w("A single new **price-only** ranking factor, the **52-Week-High Proximity**:\n")
    w("```")
    w("score = current_close / max(close over the last 252 trading days)")
    w("```")
    w("Range (0, 1]; a score of 1.0 means the stock is sitting at its one-year high, "
      "higher = better rank. It is a *level* (anchoring) signal rather than a "
      "path-dependent cumulative return — the property that, per George & Hwang "
      "(2004, *Journal of Finance*), carries momentum's under-reaction edge with "
      "**lower crash risk**. This is a direct attempt to address the deep momentum "
      "drawdown measured earlier on this universe.\n")
    ...
    w("\nVariants **C** and **D** are produced by the *same* validated 50/50 "
      "momentum+low-vol blend (`make_blend_scorer`); they differ **only** in the "
      "ranking leg (12-1 momentum vs 52-week-high). So D minus C isolates the factor "
      "and nothing else.\n")

    w("## 3. Headline results (Top10 / quarterly / Buffer20, net)\n")
    w("| Variant | CAGR | MaxDD | Sharpe | Calmar | Alpha | Vol | Beta | Trades | Turnover | Costs (Rs) |")
    w("|---|---|---|---|---|---|---|---|---|---|---|")
    for n in names:
        w(mrow(n))
    ...
    w(f"Realized span split at its midpoint ({mid_date.date()}); each half measured "
      "independently. A factor that is real should not live in only one half.\n")
    ...
    w("Top3 / Top5 / Top10 for each variant. This is a **robustness read, not a "
      "tuning knob** — Top10 remains the headline and N is not optimized.\n")
    ...
    w("\n## 8. Caveats\n")
    w("- **Beta/Alpha are unreliable** and shown only because they were requested. The "
      "benchmark *daily-return* series (`get_benchmark_returns`) is corrupted — "
      "annualised vol ~77% with spurious ±30–40% daily jumps caused by a date-parse "
      "ambiguity (`dayfirst`/`mixed`) in `nifty500_tri.csv`. That inflates benchmark "
      "variance, collapsing Beta toward ~0 and overstating Jensen's Alpha (≈ CAGR − "
      "rf). It is a **pre-existing** benchmark-data issue (the earlier Top10 runs show "
      "the same +12.8% alpha), and per the brief the **benchmark methodology was not "
      "modified**. The CAGR-endpoint benchmark is unaffected, so **Excess vs Bmk "
      "(CAGR)** is the trustworthy relative metric and all conclusions here rest on "
      "CAGR / Sharpe / drawdown, not on Alpha/Beta. _Recommended separate fix: repair "
      "the benchmark CSV date parsing (out of scope for this frozen-engine task)._\n")
    w("- 52WH here uses **daily closes** for both the current price and the 1-year "
      "high (self-contained on the close panel). Using intraday highs is a defensible "
      "alternative spec, not a tuning parameter — left for a separate robustness run.\n")
    ...
    w("## 9. Reproduce\n")
    w("```")
    w("py run_52week_high.py")
    w("```")
    w("Outputs: `results/factor_52week_high/{comparison.csv, concentration.csv, "
